  ...
  try {
    ...
    if (!embeddingData.data || !embeddingData.data[0]) {
      return {
        ...
        body: JSON.stringify({ error: 'OpenAI 임베딩 실패: ' + JSON.stringify(embeddingData).slice(0, 200) })
      };
    }
...
5단계: 종합 정리`
    ...
    if (!claudeData.content || !claudeData.content[0]) {
      return {
        statusCode: 500,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ error: 'Claude 응답 실패: ' + JSON.stringify(claudeData).slice(0, 300) })
      };
    }
